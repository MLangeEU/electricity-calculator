...
def konfigurieren():
    ...
    window.geometry("510x350")  # Size of the window
...
def berechnen():
    try:
        ...
        result_label.config(
            text=f"Energy costs per month: {costs_month:.2f} Euro\n"
                 f"Energy costs per year: {costs_year:.2f} Euro",
            justify="center"
        )
    except ValueError:
        result_label.config(text="Please use only numbers from 1-9!")

# Create main window
window = tk.Tk()
window.title("Energy costs calculator")

# Open configuration
konfigurieren()

# Labels for input fields
label_consum = tk.Label(window, text="Consumption in watt:", font=("Helvetica", 15), foreground="white", background="#2E2E2E")
label_efficiency = tk.Label(window, text="Efficiency in %:", font=("Helvetica", 15), foreground="white", background="#2E2E2E")
label_hours = tk.Label(window, text="Hour per day:", font=("Helvetica", 15), foreground="white", background="#2E2E2E")
label_days = tk.Label(window, text="Numbers of days:", font=("Helvetica", 15), foreground="white", background="#2E2E2E")
label_costs = tk.Label(window, text="Electricity costs in Cent/kWh:", font=("Helvetica", 15), foreground="white", background="#2E2E2E")
...
calculate_button = tk.Button(window, text="Calculate", command=berechnen, font=("Helvetica", 14), fg="white", bg="#3A3A3A", 
